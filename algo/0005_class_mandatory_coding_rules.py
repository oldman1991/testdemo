# coding=utf-8
# create by oldman at 2017/11/8
"""
问题：
    你的代码包含一个很大的类继承体系，你希望强制执行某些编程规约(或者代码诊断)来帮助程序员保持清醒

解决方案：
    如果你想监控类的定义，通常可以通过定义一个元类。一个基本原来通常是继承自 type
    并重定义它的 __new__() 方法或者是 __init__()方法。比如：
"""
...
class MyMeta(type):
    def __new__(cls, clsname, bases, clsdict):
        # claname is name of class being defines
...
"""
为了使用这个元类，你通常要将它放到一个丁记父类定义中，然后其他的类继承这个顶级父类，
例如：
"""
...
"""
元类的一个关键特点是它允许你在定义的时候检查类的内容。在重新定义__init__()方法中，
你可以很轻松的检查类字典，父类等等。并且，一旦某个元类被指定给了某个类，那么就会被继承到
所有子类中区。因此，一个框架的构建者就能在大型的继承体系中通过给一个顶级父类制定一个元类去捕获
所有下面子类的定义。
作为一个具体的应用例子，下面定义了一个元类，它会拒绝任何有混合大小写名字作为方法的类定义(可能想气死java程序员。哈哈哈、、、)
"""


class MoMixedCaseMeta(type):
    def __new__(cls, clsname, bases, clsdict):
        for name in clsdict:
            if name.lower() != name:
...
class Root1(metaclass=MoMixedCaseMeta):
    pass
...
class B1(Root1):
    def foo_Bar(self):  # TypeError
        pass


"""
作为更高级和实用的例子，下面有一个元类，它用来检测重载的方法，确保它的调用
参数跟父类中原始方法有着相同的参数签名
"""
...
class Root2(metaclass=MatchSignatureMeta):
    pass
